// P10: deploy v4 — the hardened graph PLUS the on-chain SVG renderer — to
// Nile over the user's Chrome. p8 flow with: 13 files (IRenderer +
// CardRenderer joined), a second deploy (CardRenderer), setRenderer wiring,
// and a two-layer tokenURI decode check. TronLink popups auto-confirmed
// (Nile testnet; only the txs this script itself initiates).
const fs = require('fs')
...
const OLD_DEPLOYS = ['TEzyMokXwNqJteoSGC1v4rerK4mkfYE1f9', 'TYK5P6bUBGuadpjyB9aZ6nVSDEj98PfSWR']
const FILES = [
...
const notes = []
const log = (m) => console.log('[p10]', m)
const note = (m) => { notes.push(m); console.log('[p10][NOTE]', m) }
...
;(async () => {
  ...
  const pinAndCompile = async () => {
    ...
    log('compiled the 13-file v4 graph on ' + (v0820 || 'current version'))
  }
  ...
  log('DEPLOY ThreeRealmsCards (v4) — auto-confirming TronLink…')
  ...
  if (!isTAddr(cardsAddr) || OLD_DEPLOYS.includes(cardsAddr)) {
    ...
    throw new Error('could not identify the fresh v4 address')
  }
  log('cards (v4) deployed at ' + cardsAddr)
  fs.writeFileSync(SCRATCH + '/p10-cards-addr.txt', cardsAddr)
  ...
  await page.screenshot({ path: SCRATCH + '/p10-genesis.png' })
  ...
  console.log('P10-OK')
})().catch((e) => { console.error('P10-FAIL', e); process.exit(1) })
