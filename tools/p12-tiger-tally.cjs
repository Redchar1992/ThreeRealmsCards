// P12: 虎符演武 — deploy TigerTally against the live v5 cards, seat it on the
...
;(async () => {
  ...
  // ---------------- workspace: idempotent 14-file write
  ...
  if (wrote.written > 0) { await page.waitForTimeout(6000) } // flush; NO reload needed for compile-by-path? the tree needs it —
  // TigerTally.sol is new: reload so the tree sees it, then re-arm the overlay guard
  if (wrote.written > 0) {
  ...
  // ---------------- open TigerTally.sol + compile its graph
  ...
  log('compiled the TigerTally graph')
  ...
  log('DEPLOY TigerTally(' + CARDS_B58.slice(0, 8) + '…) — auto-confirming TronLink…')
  await page.locator('button[data-id="Deploy - transact (not payable)"]', { hasText: 'Deploy' }).click({ force: true })
  await untilVisible(page.locator('.instance').first(), 240_000, 'tally instance')
  const tallyAddr = await page.evaluate(() => {
    let found = ''
    for (const e of Array.from(document.querySelectorAll('[data-id="recorderAddressBookEntry"]'))) {
      ...
      if (nm && nm.innerText.trim() === 'TigerTally') { const ad = e.querySelector('[data-id="recorderAddressBookAddress"]'); if (ad) found = ad.innerText.trim() }
    }
    return found
  })
  if (!/^T[1-9A-HJ-NP-Za-km-z]{33}$/.test(tallyAddr)) throw new Error('no tally address: ' + tallyAddr)
  log('TigerTally deployed at ' + tallyAddr)
  ...
  const instFor = async (marker) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (txt.includes(marker)) return inst
    }
    throw new Error('instance not found for ' + marker)
  }
  ...
  const tallyHex = b58ToHex20(tallyAddr)
  let seated = false
  for (let i = 0; i < 20; i++) {
    const res = await constCall(CARDS_B58, 'suzerain()')
    if (res.endsWith(tallyHex.slice(2))) { seated = true; break }
  ...
  for (const c of candidates) {
    const h = ethers.TypedDataEncoder.hashDomain({ name: 'Three Realms Tiger Tally', version: '1', chainId: c, verifyingContract: tallyHex })
  ...
  const canonicalDigest = ethers.TypedDataEncoder.hash(
    { name: 'Three Realms Tiger Tally', version: '1', chainId, verifyingContract: tallyHex },
    TYPES, orderForDigest
  )
  const digestOnChain = '0x' + (await constCall(tallyAddr, 'digestOf((address,(string,uint8,uint8,uint8,uint8,uint8,uint8,uint8,string),uint256,uint64))',
    ethers.AbiCoder.defaultAbiCoder().encode(
      ['tuple(address,tuple(string,uint8,uint8,uint8,uint8,uint8,uint8,uint8,string),uint256,uint64)'],
  ...
  const variants = [
    ...
    { label: '0x addrs', domainAddr: tallyHex, to: '0x' + '0'.repeat(40) },
