...
## 2026-07-14 · P13:市集全周期 —— 真金白银闭环

**合约**(commit `13f6530`):`CardBazaar` 最小诚实市场——托管式固定价挂单(无陈旧挂单)、恰好价成交、**pull-payment 提款**(内联打款会让 `receive()` 回滚的卖家卡死自己挂单的成交;pull 把买家的交易与卖家的收款能力彻底解耦)、`withdraw` 走 CEI(先清账本再动钱)、`call{value:}` 弃 `.transfer`(能量计价的 VM 不需要固定 gas stipend 这种脆性)、拒收裸 safe 投递、**零治理面**(无 owner 无费用无 sweep,信任模型管理员栏=nobody)。测试 92→111,覆盖率保持全 100%;`HostileSellerMock` 两种人格:重入型在收款瞬间尝试二次提款——只看到已清零的账本、单次收款实证;拒收型提款失败但分文不损,且**它的挂单照样能成交**——pull-over-push 的全部论证浓缩成一条测试。

**Nile 演武**(`TKfYhL4AQvR5zHxaWezFN4ve7PChsEB6RU`):真实 TRX 全周期一次通过、零备注——approve → `list(#3, 100 TRX)`(张飞入柜,摊位链上可查)→ `buy` 恰好价(卡归买家、市集余额 100 TRX、待提款 100 TRX)→ `withdraw`(账本与余额双归零)。四步全部 TronGrid 直查验证;部署地址直接取自链上回执(p12 教训固化:地址簿与实例标题都不是可靠叙事者)。

**扩展战役至此四连收官**:渲染层(视觉)、虎符(签名)、市集(价值)、加固核心(标准)——代码库的四个能力维度各有一个上链实证的参考实现,111 测试 100% 覆盖,J 台账 15 条,五个版本、四份在役合约。

## 2026-07-14 · P14:桃园馆 v2 —— 时间维度补全,五连收官

**合约**(commit `23cfd73`):桃园馆原地演进 v2——`depositGift(tokenId, heir, claimBy)` 期限严格未来;**无缝无叠的边界规则**:继承人窗口 `now <= claimBy`(边界秒归继承人),悔赠窗口 `now > claimBy` 且仅赠予人;期限内不可悔赠(随时能收回的不叫赠礼);过期赠礼无悔赠期限,在托管里永远等它的主人。顺手关掉 v1 陷阱:heir=address(0) 与空位哨兵碰撞会把卡永久卡死——v2 直接拒绝(`HeirlessGift`)。测试 111→120,覆盖率保持全 100%,`evm_setNextBlockTimestamp` 打点边界秒双向翻转矩阵。

**Nile 演武**(`TTSyQWDjVbtWv9DuR74HF8fLX9Wsi3iX62`,**桃园馆首次上链**——v1 时代始终只在 VM):①+1h 赠礼→即领(claim 路径);②+90s 赠礼→**期限内 `triggerconstantcontract` 模拟悔赠,被拒且回执解码出 `GiftStillClaimable`**——常量模拟证伪状态变更调用,零成本、零失败交易,这个姿势值得沉淀——→真实等待 110s→悔赠落地、礼位清空(regret 路径)。

**过程小坑**:TronLink 出现新版 `secondary_popup` 签名窗,旧确认器选择器未命中,弹窗超时自关、部署作废(链上无痕,干净重试)——确认器升级为 role+正则正向匹配(confirm/sign/approve/确认/签名/同意),且确认泵贯穿回执轮询期。另观察到部署账户 19:00-19:06 有人工痕迹交易(转账+能量委托),自动化与真人共用钱包时段的礼让问题记录在案。

**五连收官**:标准(加固核心)、视觉(丹青)、签名(虎符)、价值(市集)、时间(桃园馆 v2)——五个能力维度各有链上实证;120 测试全 100% 覆盖;v5 生态五合约在役。

## 2026-07-14 · P15:验证双栈 —— Foundry 不变量战役,工程收官

**接线**(零 submodule、零 vendor):npm 的 forge-std 镜像停在 1.1.2(史前版本,无 StdInvariant),git submodule 又要为六个函数背四十个文件——最终**自写 60 行 `ForgeLite` 垫片**(Vm 接口子集、bound/makeAddr、targetContracts/targetSelectors 注册约定),零外部依赖延伸到测试基座本身。`foundry.toml` 与 hardhat 产物目录隔离(out-forge/cache-forge),solc 0.8.20 + paris 双栈同参。
