...
```text
Card (storage)
  → CardCodec.toTokenURI(card, tokenId)          [library using-for]
      → attributes JSON  (faction/rarity names, four stats via Str.toString)
      → top-level JSON   (general & series via Str.escapeJson)
  → Base64.encode(json)
...
- **Everything on chain.** The URI is self-contained; wallets decode it with
  no network fetch. The cost is paid once at read time (views are free on
  TRON) and the metadata shares the chain's lifetime.
- **Escaping at read time, not mint time.** Minting accepts any UTF-8 name;
  `escapeJson` guarantees the JSON stays valid (`"`, `\`, control chars). The
  clean-string fast path returns the input untouched.
- **No image field yet.** A deliberate scope cut while the project is a
  reference implementation rather than a consumer product; the pipeline
  composes with an on-chain SVG or IPFS/BTFS pointer without structural
  change.
...
| an `image` in metadata | see pipeline notes above |
